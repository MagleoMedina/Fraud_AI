...
def generar_dataset_csv(n_muestras=300, seed=42, ruta_base='dataset/dataset'):
    """
    Genera un dataset simulado para detección de fraude y lo guarda en archivos CSV.
    Divide el dataset en dos partes (60% y 40%) y realiza sobremuestreo de fraudes.
    """
    np.random.seed(seed)
    ...
    # Generar datos simulados para cada característica
    ...
    # Crear DataFrame original con los datos simulados
    df = pd.DataFrame({
        'monto': monto,
    ...
    # Sobremuestreo manual de fraudes para mejorar el balance de clases
    ...
    # Dividir el dataset en 60% y 40% para entrenamiento y prueba
...
if __name__ == "__main__":
    # Llama a la función para generar los datasets si el script se ejecuta directamente
    generar_dataset_csv()
